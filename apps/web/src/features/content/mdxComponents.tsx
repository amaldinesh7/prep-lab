...
export const mdxComponents = {
  ...
  h2: (props: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h2 className="text-[26px] font-semibold mt-14 mb-5" {...props} />
  ),
  p: (props: React.HTMLAttributes<HTMLParagraphElement>) => (
    <p className="mb-5 max-w-[64ch]" {...props} />
  ),
  ul: (props: React.HTMLAttributes<HTMLUListElement>) => (
    <ul className="list-disc pl-5 mb-5 max-w-[64ch] text-[var(--text-muted)]" {...props} />
  ),
  code: (props: React.HTMLAttributes<HTMLElement>) => (
    <code className="bg-[var(--hover)] border border-[var(--border)] rounded-[3px] px-1.5 py-0.5 text-[0.88em]" {...props} />
  ),
  // custom
  Callout,
  Code,
